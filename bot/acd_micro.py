...
class InstrumentSpec:
    """Everything instrument-specific. Fisher's A/C values are ~18-20% of the ATR (his exact
    stated formula, per the 2018 webinar) -> set a_atr_frac/c_atr_frac and pass atr into
    build_day. If atr/a_atr_frac are absent we fall back to a_pct/c_pct (% of OR midpoint)."""
    symbol: str = "SPX"
    or_minutes: int = 15
    a_pct: float = 0.0018           # fallback: A value as % of price (used only if atr is None)
    c_pct: float = 0.0021
    a_atr_frac: float = None        # Fisher: A value = a_atr_frac * ATR  (~0.18)
    c_atr_frac: float = None        # Fisher: C value = c_atr_frac * ATR  (C > A)
...
# ---------------------------------------------------------------- Stage 1: levels
def compute_levels(bars, prior_hlc, spec, atr=None):
    """bars = [(t 'HH:MM', price)]; prior_hlc = (High, Low, Close) of the prior day.
    A/C values are Fisher's ~18-20% of the ATR when atr + spec.a_atr_frac are given
    (the faithful formula); otherwise the % of OR-midpoint fallback."""
    or_end = _add_minutes(spec.session_open, spec.or_minutes)
    hi, lo = opening_range(bars, spec.session_open, or_end)
    if atr is not None and spec.a_atr_frac is not None:
        a, c = spec.a_atr_frac * atr, spec.c_atr_frac * atr      # Fisher: A/C ~= 18-20% of ATR
    else:
        mid = (hi + lo) / 2.0
        a, c = mid * spec.a_pct, mid * spec.c_pct                # fallback: % of price
...
def _resolve_failure(bars_sorted, t_min, beyond, hold_min):
    """Time+price at which a pierce is CONFIRMED failed — the first bar within the hold
    window where price is back INSIDE the level (not `beyond`). Returns None if the pierce
    instead held (still beyond at hold_min) OR never resolved within the data (bars ran out
    while still beyond). A fade is only live-tradeable at THIS moment/price — not at the
    pierce bar (the extreme), which is unknowable to be a 'failure' until it snaps back.
    Mirrors _check_hold's control flow so hold/fail classification stays consistent."""
    for t2, p2 in bars_sorted:
        tm2 = _to_min(t2)
        if tm2 < t_min:
            continue
        if tm2 - t_min >= hold_min:
            return None if beyond(p2) else (t2, p2)     # window elapsed: held if beyond, else fail here
        if not beyond(p2):
            return (t2, p2)                             # snapped back inside within window -> fail here
    return None                                         # ran out of bars still beyond -> not a confirmed failure


def detect_a_events(bars, lv, spec):
    """A_up/A_down (held, one per day) + the first CONFIRMED failed_A of each side, after the
    OR window and at/before the cutoff. A failed_A is stamped at its failure-resolution bar
    (snap-back), not the pierce, so the fade entry is live-knowable and fills at a real price."""
    ...
    for t, price in bs:
        tm = _to_min(t)
        if tm < or_end_m or tm > cut_m:                 # OR end EXCLUSIVE -> the or_end bar is post-OR
            continue
        ...
        if confirm is not None:
            ...
            if fail is not None:                                # only a CONFIRMED failure is a live-tradeable fade
                events.append(ACDEvent("failed_" + typ, fail[0], fail[1], False))
                failed[typ] = True
    return events
    ...
    for t, p in bs:
        ...
        if c_beyond(p):
            ...
            if confirm is not None:
                ...
                fail = _resolve_failure(bs, _to_min(t), c_beyond, hold_min)   # snap-back (time, price)
                if fail is not None:
                    events.append(ACDEvent("failed_" + c_typ, fail[0], fail[1], False))
                    failed_c = True
...
def setups_from_c(c_event, lv, spec, close):
    ...
    if through_pivot(direction, level, lv.pivot_band, lv.or_high, lv.or_low):
        ...
        out.append(Setup("late_day_c" if late else "c_through_pivot", direction,
                         c_event.time, c_event.price, lo if direction == "long" else hi, 3,
                         "overnight" if late else "intraday", {"pivot_band": lv.pivot_band}))
    return out
        ...
        if e.type == "failed_A_up" and not held_by(held_up_times, _to_min(e.time)):   # fade a failed up-breakout -> short
            if lo <= lv.a_up <= hi:
                out.append(Setup("failed_a_pivot", "short", e.time, e.price, hi, 2, "intraday",
                                 {"pivot_band": lv.pivot_band, "a": lv.a_up}))
            else:
            ...
            if lo <= lv.a_down <= hi:
                out.append(Setup("failed_a_pivot", "long", e.time, e.price, lo, 2, "intraday",
                                 {"pivot_band": lv.pivot_band, "a": lv.a_down}))
            else:
...
# ---------------------------------------------------------------- orchestrator
def build_day(date, bars, prior_hlc, spec=SPX, atr=None):
    lv = compute_levels(bars, prior_hlc, spec, atr)
...
if __name__ == "__main__":
    ...
    # --- failed A up -> failed_a short, entered at the SNAP-BACK (resolution) bar/price, NOT the A extreme ---
    # Live you can't know the pierce "failed" until price comes back inside; you fade THERE, at THAT price.
    fa = OR + [("09:50", 5020), ("09:52", 5000)]        # pierce a_up (~5019) at 09:50, snaps back inside at 09:52
    d = build_day("D2", fa, PIV_BELOW)
    fe = next(e for e in d.events if e.type == "failed_A_up")
    assert fe.time == "09:52" and abs(fe.price - 5000) < 1e-9, fe    # stamped at resolution 09:52/5000, not 09:50/5020
    assert not any(e.type == "A_up" and e.held for e in d.events)
    fs = [s for s in d.setups if s.name == "failed_a"][0]
    assert fs.direction == "short" and fs.entry_time == "09:52", fs  # enter at the snap-back bar
    assert abs(fs.entry_price - 5000) < 1e-9, fs                     # ...at the snap-back price (not the ~5019 A extreme)
    assert abs(fs.stop - 5019.0172) < 1e-2, fs                       # stop still just above the A level
    print("failed_a OK: fade entered at the snap-back resolution bar/price (not the A extreme)")
