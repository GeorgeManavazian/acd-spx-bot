...
def trt(i, history):
    ...
    if state == "trend_up" and e.ohlc[0] > max(x.ohlc[0] for x in recent) and \
            "A_down" in held and "failed_C_up" in types:
        return Setup("trt", "short", "16:00", e.ohlc[2], None, 3, "eod", {"note": "approx (no open/gap)"})
    if state == "trend_down" and e.ohlc[1] < min(x.ohlc[1] for x in recent) and \
            "A_up" in held and "failed_C_down" in types:
        return Setup("trt", "long", "16:00", e.ohlc[2], None, 3, "eod", {"note": "approx (no open/gap)"})
    return None
...
def sushi(i, history, n=5):
    ...
    if cur_hi > prev_hi and cur_lo < prev_lo:
        # sushi is a close-beyond-the-prior-window pattern -> known only AT the close -> EOD setup
        # (was mislabeled "intraday" with a date as entry_time).
        if close < prev_lo:
            return Setup("sushi", "short", "16:00", close, None, 3, "eod",
                         {"prev": (prev_lo, prev_hi)})
        if close > prev_hi:
            return Setup("sushi", "long", "16:00", close, None, 3, "eod",
                         {"prev": (prev_lo, prev_hi)})
    return None
...
def macro_context(i, history):
    ...
    if j < 0:                                   # first day: no prior context -> neutral, no gating
        cum, state, regime, mom, pm = 0, "neutral", "neutral", 0, 0
    else:
        cum, state = number_line_state(history, j)
        regime = pivot_ma_regime(history, j)
        mom = momentum(history, j)
        pm = plus_minus(history[j])
    macro = [s for s in (reversal_trade(i, history), trt(i, history), sushi(i, history)) if s]
    return MacroContext(e.date, score_day(e), cum, state, regime, mom, pm, macro)
