...
def detect_breakout(path, high, low, range_end="10:30", cutoff="12:00"):
    ...
    for t, spot in sorted(path):              # chronological by "HH:MM"
        if t < range_end or t > cutoff:       # OR end is exclusive -> the range_end bar is tradeable
            continue
        if spot > high:
...
def build_orb(path, strikes, expiration, width=15.0, buffer=0.0,
    ...
    option data at the entry minute (see orb_exits).
    """
...
if __name__ == "__main__":
    ...
    chop = [("09:30", 5005), ("10:00", 5002), ("11:00", 5004), ("12:00", 5003)]   # 2 bars INSIDE the (exclusive-end) OR
